...
import { FiTrendingUp, FiExternalLink, FiFilter, FiRefreshCw, FiZap, FiRadio } from 'react-icons/fi';
...
const News = () => {
  ...
  const fetchNewsAndSuggestions = async () => {
    try {
      ...
      if (industries.length > 0) queryParams.append('industries', industries.join(','));
      if (instruments.length > 0) queryParams.append('instruments', instruments.join(','));

      const suggRes = await fetch(`http://localhost:5000/api/portfolio/suggestions?${queryParams.toString()}`);
      if (suggRes.ok) {
  ...
  // Market Pulse Marquee Data
  const marketIndices = [
    { name: 'NIFTY 50', value: '25,388.90', change: '+0.42%', positive: true },
    { name: 'SENSEX', value: '83,184.80', change: '+0.36%', positive: true },
    { name: 'S&P 500', value: '5,634.58', change: '+0.75%', positive: true },
    { name: 'NASDAQ', value: '17,713.62', change: '+1.00%', positive: true },
    { name: 'GOLD (10g)', value: '₹73,420', change: '+0.18%', positive: true },
    { name: 'USD / INR', value: '₹83.72', change: '-0.05%', positive: false }
  ];

  // Running Breaking Headlines
  const breakingHeadlines = [
    "RBI Monetary Policy Committee maintains Repo Rate at 6.50% citing sustained disinflation trajectory",
    "Apple unveils next-generation M-Series chips with dedicated AI accelerators; Analysts raise target to $250",
    "Reliance Industries partners with global sovereign wealth funds for ₹75,000 Cr renewable energy rollout",
    "Indian mutual fund AUM crosses ₹65 Lakh Crore record as retail monthly SIPs breach ₹23,000 Cr",
    "Federal Reserve signals 50 bps benchmark interest rate cut path as headline inflation cools to 2.5%",
    "HDFC Bank reports 17.5% YoY loan growth driven by digital retail underwriting and MSME credit lines",
    "Brent Crude softens below $72/barrel easing import bill pressures for emerging market economies"
  ];
          ...
          {marketIndices.map((idx, i) => (
            <div key={i} style={{ background: 'var(--bg-color)', padding: '0.6rem 0.8rem', border: '1px solid var(--border-color)' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginTop: '0.2rem' }}>
                ...
                <span style={{ fontSize: '0.75rem', fontWeight: 600, color: idx.positive ? '#064E3B' : '#B91C1C' }}>
                  {idx.change}
                </span>
              </div>
            </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div style={{ background: 'var(--surface-color)', border: '1px solid var(--border-color)', padding: '1.5rem' }}>
            ...
            <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', margin: '0 0 1.25rem 0', lineHeight: 1.4 }}>
              ...
              {userPreferences.industries?.length > 0 ? (
                ...
                <span style={{ display: 'block', fontStyle: 'italic', marginTop: '0.2rem' }}>
                  Technology, Banking & Index Mutual Funds
                </span>
              )}
            </p>
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <div style={{ fontSize: '0.75rem', fontWeight: 600, color: '#064E3B' }}>{item.change}</div>
                    </div>
                  </div>
            ...
            <div style={{ marginTop: '1.25rem', borderTop: '1px solid var(--border-color)', paddingTop: '1rem', textAlign: 'center' }}>
              <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>
                Suggestions update dynamically with market volatility.
              </span>
            </div>
          </div>
          ...
          <div style={{ background: 'var(--surface-color)', border: '1px solid var(--border-color)', padding: '1.5rem' }}>
            ...
            <p style={{ margin: 0, fontSize: '0.8rem', color: 'var(--text-secondary)', lineHeight: 1.5 }}>
              Holding a combination of domestic blue chips (e.g. Reliance, HDFC) and global tech leaders (e.g. Apple, Nvidia) alongside broad index funds ensures maximum compound growth with mitigated drawdown risk.
            </p>
          </div>
